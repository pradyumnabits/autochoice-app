...
SQLALCHEMY_DATABASE_URL = f"sqlite:///{os.path.join(db_directory, 'roadside_assistance.db')}"

# Function to initialize the database
def initialize_database():
    try:
        with sqlite3.connect(os.path.join(db_directory, 'roadside_assistance.db')) as conn:
            cursor = conn.cursor()
...
def create_request(request: RoadsideAssistanceRequest):
    ...
    cursor.execute(
        ...
        (request_id, request.user_id, request.vehicle_id, request.location, "Pending", None)
    )
    conn.commit()
    conn.close()
    return request_id
...
def get_all_roadside_requests():
    ...
    return [{"id": r[0], "user_id": r[1], "vehicle_id": r[2], "location": r[3], "status": r[4], "provider": r[5]} for r in requests]
...
def get_roadside_status(requestId: str):
    ...
    return {"id": request[0], "user_id": request[1], "vehicle_id": request[2], "location": request[3], "status": request[4], "provider": request[5]}
