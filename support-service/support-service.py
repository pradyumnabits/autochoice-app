...
SQLALCHEMY_DATABASE_URL = f"sqlite:///{os.path.join(db_directory, 'service_support.db')}"
...
def create_service_appointment(appointment: ServiceAppointment):
    ...
    cursor.execute(
        ...
        (appointment.user_id, appointment.vehicle_id, appointment.appointment_date, appointment.service_type)
    )
    conn.commit()
    conn.close()
...
def get_service_history(user_id: str, vehicle_id: str = None):
    ...
    cursor = conn.cursor()
    if vehicle_id:
        cursor.execute("SELECT * FROM service_history WHERE user_id = ? AND vehicle_id = ?", (user_id, vehicle_id))
    else:
        cursor.execute("SELECT * FROM service_history WHERE user_id = ?", (user_id,))
...
def list_service_history(user_id: str, vehicle_id: str = None):
    ...
    return [{"user_id": h[1], "vehicle_id": h[2], "service_date": h[3], "service_type": h[4], "description": h[5]} for h in history]

@app.get("/service/appointments")
def list_service_appointments():
    appointments = get_all_service_appointments()
    return [{"id": a[0], "user_id": a[1], "vehicle_id": a[2], "appointment_date": a[3], "service_type": a[4], "status": a[5]} for a in appointments]
