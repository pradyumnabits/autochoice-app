...
def create_customer(customer: Customer):
    ...
    cursor.execute(
        ...
        (customer.userName, customer.firstName, customer.lastName, customer.email, customer.phoneNumber, customer.address, customer.profileStatus)
    )
    conn.commit()
    conn.close()
...
def update_customer(userName: str, customer: Customer):
    ...
    cursor.execute(
        ...
        (customer.firstName, customer.lastName, customer.email, customer.phoneNumber, customer.address, customer.profileStatus, userName)
    )
    conn.commit()
    conn.close()
...
def update_customer_status(userName: str, status: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE customers SET profileStatus = ? WHERE userName = ?",
        (status, userName)
    )
    conn.commit()
    conn.close()
...
def list_customers():
    ...
    return [{"userName": c[0], "firstName": c[1], "lastName": c[2], "email": c[3], "phoneNumber": c[4], "address": c[5], "profileStatus": c[6]} for c in customers]
...
def get_customer(userName: str):
    ...
    return {"userName": customer[0], "firstName": customer[1], "lastName": customer[2], "email": customer[3], "phoneNumber": customer[4], "address": customer[5], "profileStatus": customer[6]}
