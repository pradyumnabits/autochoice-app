...
app = FastAPI(title="Customer Feedback Service API", version="1.0.0", description="API for collecting and managing customer feedback for vehicles and post-sale services.")
...
def create_feedback(feedback: FeedbackRequest):
    ...
    cursor.execute(
        ...
        (feedback_id, feedback.user_id, feedback.feedback_type, feedback.reference_id, feedback.details, feedback.rating, submitted_at)
    )
    conn.commit()
    conn.close()
...
def submit_feedback(feedback: FeedbackRequest):
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid feedback submission request")

@app.get("/feedback/{id}", response_model=FeedbackResponse, responses={404: {"model": ErrorResponse}})
